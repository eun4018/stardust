...
!function($) {
  ...
  window.myChart = function () {
    ...
    /*REP_001_0002_15.html card 14_3, card 14_4, card 15_1, card 15_2 */
    let SubDonut2 = function (set) {
      CHART(function (a11y) {
        ...
        let foreGround = group.append("path")
          ...
          .attrs({"class": "foreground-" + set.case, "d": arc1})
          .transition().duration(500).ease(d3.easeExp)
          .attrTween("d", arcTween(pieGenerator([100 - set.data[0],set.data[0]]))) //변경
          .delay(600);
        ...
        let Text = group.append("text")
          .attrs({
            "dy": set.case !== "case-1" ? "10px" : "20px",
            // "class": "sub-text-1"
            "class": (set.case == "case-2" && set.data[0] ) ? "sub-text-2" : "sub-text-1"
          });
        ...
        function arcTween (pie) {
          return function (d) {
            let interpolate = d3.interpolate(-0, -pie[0].endAngle);
            // let interpolateText = d3.interpolate(0, set.data[0]);
            let interpolateTextData = (set.case == "case-2" && set.data[0] < 50) ? set.data[0] : set.data[0]//변경 
            let interpolateText = d3.interpolate(0, interpolateTextData);
            return function (t) {
              d.endAngle = interpolate(t);
              Text.text(format(interpolateText(t)) + '%');
