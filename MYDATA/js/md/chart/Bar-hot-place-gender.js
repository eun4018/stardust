    ...
    new Vue({
      ...
      <div
        ...
        <div class="chartGroup">
          ...
          <!-- // WAI-ARIA -->
            <em>방문자 분석 성별/연령대별</em>
            <p>성별/연령대별 방문 비율 정보</p>
          </div>
          ...
          <ul class="x-area" aria-level="X축 범위">
            <li
              ...
              <span class="bar" ref="valueHight">
                <em 
                  v-for="(gender, index_2) in item.gender" 
                  :key="gender.id"
                  :aria-label="gender.name" 
                  :aria-valuenow="gender.value"
                  :aria-pressed="item.pressed"
                  v-on:click="selectedClick(index_1, index_2)"></em>
                <div 
                  v-for="(gender, index) in item.gender" 
                  class="tooltip"
                  :class="{selected: gender.selected}"
                  :style="{bottom: _value(gender.value) + 'px'}"><span>{{gender.name}} {{gender.value}}%</span></div>
              </span>
      ...
      created: function () {
        if (typeof this.xArea == 'object') {
          this.xArea.map(function (value, index) {
            ...
            value.gender.map(function (data, index) {
              Object.assign(data, {selected: true, pressed: true})
        ...
        let maxList = this.xArea.map(function (data) {
          return data.gender.map(function (data) {
            return data.value;
          });
        });
          ...
          handler: function () {
            ...
            if (typeof _xData == 'object') {  
              _xData.map(function (value, index_1) {
                let n1 = index_1;
                value.gender.map(function (data, index_2) {
                  let n2 = index_2;
                  let n2_3 = index_2 + 3;
